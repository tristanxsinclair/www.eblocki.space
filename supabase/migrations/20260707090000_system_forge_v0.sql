-- System Forge v0: custom proof systems and first-rep history.
-- Proof artifacts remain in public.proof_artifacts; system_reps links to them.
...
CREATE INDEX IF NOT EXISTS idx_custom_systems_user_active
ON public.custom_systems(user_id, is_active, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_system_reps_user_system_time
ON public.system_reps(user_id, system_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_system_reps_proof
ON public.system_reps(proof_id);
...
CREATE POLICY "system_reps select own"
ON public.system_reps
FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "system_reps insert own" ON public.system_reps;
CREATE POLICY "system_reps insert own"
ON public.system_reps
FOR INSERT
WITH CHECK (
  auth.uid() = user_id
  AND EXISTS (
    SELECT 1
    FROM public.custom_systems
    WHERE custom_systems.id = system_reps.system_id
      AND custom_systems.user_id = auth.uid()
  )
  AND (
    proof_id IS NULL
    OR EXISTS (
      ...
      WHERE proof_artifacts.id = system_reps.proof_id
        AND proof_artifacts.user_id = auth.uid()
    )
  )
);

DROP POLICY IF EXISTS "system_reps update own" ON public.system_reps;
CREATE POLICY "system_reps update own"
ON public.system_reps
FOR UPDATE
USING (auth.uid() = user_id)
WITH CHECK (
...
CREATE TRIGGER trg_custom_systems_updated
