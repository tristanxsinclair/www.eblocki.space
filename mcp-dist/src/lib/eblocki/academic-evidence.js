...
exports.ACADEMIC_DIMENSIONS = void 0;
exports.evidenceOnly = evidenceOnly;
exports.academicEvidence = academicEvidence;
exports.academicGap = academicGap;
...
exports.ACADEMIC_DIMENSIONS = ["explanation", "application", "discrimination", "correction"];
...
function academicGap(evidence) {
    if (!evidence.explanation)
        return { target: "explanation", gap: "A worked concept explanation is not yet visible.", action: "Explain one concept in a full answer, including why it works." };
    if (!evidence.application)
        return { target: "application", gap: "Definitions are visible, but a concrete scenario linked to a reason is not yet demonstrated.", action: "Answer one concrete scenario and explain why the concept accounts for the observed result." };
    if (!evidence.discrimination)
        return { target: "discrimination", gap: "Scenario application is visible; discrimination between competing explanations is not yet demonstrated.", action: "Answer one ambiguous scenario with two plausible explanations. Justify which fits better and why the alternative is weaker." };
    if (!/\b(?:dominant|more likely|stronger explanation|better explains)\b/i.test(evidence.discrimination))
        return {
